...
\subsection{Hand-Labeled Qualitative Analysis}

I sample 30 articles stratified by source length (10 short / 10 medium / 10 long) and hand-label every system's output on five binary axes: factual correctness (no fabricated entities/numbers/dates), completeness (lede preserved), Turkish fluency, morphological correctness, and absence of mode collapse. Total labeled rows: $30 \times 6 = 180$.

\paragraph{Per-system pass rates.} [Table~\ref{tab:qual} reports per-system pass rates per axis. \emph{See \texttt{outputs/results/qual\_summary.md} for the data, populated after labeling completes.}]

\paragraph{Recurring failure modes.} (a)~Teachers occasionally fabricate years; for example, a sports-retirement story whose article never mentions a year received the phrase ``2023 yılında'' in the detailed-prompt teacher output. (b)~The 1k-sample student exhibits a teacher-mimicry regime with long outputs and high hallucination rate that vanishes by 5k. (c)~The zero-shot B1 generates short noise (32\% of reference length) and fails to acquire the task without supervision. (d)~Both teachers occasionally produce mid-sentence truncations when the output exceeds the configured token cap; I raised the cap from 160 to 200 tokens during the pilot phase to mitigate this.
