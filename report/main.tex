...
\textbf{Hallucination and bias.} Teachers hallucinate numeric facts at \textbf{2--4\%} on TR-News and up to \textbf{3.9\%} on MLSUM-TR (Claude Haiku 4.5); distilled students reduce this to \textbf{0.1--0.6\%} but inherit the small fraction of teacher fabrications baked into training data. The teacher LLMs were trained on English-dominated corpora, so cultural and stylistic biases (Western framings, sporadic English vocabulary in Turkish output) likely propagate. Distilled Turkish summarizers could also be misused to generate synthetic news at scale; I do not release student weights publicly without consideration of this risk.

\textbf{Evaluation limitations.} ROUGE on Turkish systematically underestimates quality due to agglutinative-morphology fragmentation. BERTScore captures semantics but cannot detect hallucinated facts (a fluent confident lie scores high). The qualitative analysis in Section~\ref{sec:errors} is therefore essential to any deployment recommendation.

\textbf{Reproducibility.} All prompts, training configurations, evaluation scripts, and synthetic data caches are publicly available at \url{https://github.com/cagancaliskan/CENG467-term-project}. Total experimental cost was approximately \$15 in API fees against \$25 in prepaid Anthropic credit, plus approximately 14 hours of T4 GPU time on Colab Premium.
